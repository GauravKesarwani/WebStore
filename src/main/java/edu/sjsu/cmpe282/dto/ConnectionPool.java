...
public class ConnectionPool {
	...
	public static Vector<Connection> connectionList = new Vector<Connection>(
			MAX_POOL_SIZE);
	...
	public synchronized static Connection getConnection(String url,
		...
		if (connectionList.isEmpty()) {
			if (CURRENT_POOL_SIZE >= MAX_POOL_SIZE) {
				throw new MaxPoolReachedException(
						"Maximum pool size reached cannot create connections") {
...
class MaxPoolReachedException extends RuntimeException {
	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
